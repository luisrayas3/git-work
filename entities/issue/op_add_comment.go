...
// AddCommentOperation will add a new comment in the issue.
// Shape and type code are entities/bug's, so that comment ids survive the migration.
...
// AddComment is a convenience function to add a comment to an issue
func AddComment(i Interface, author identity.Interface, unixTime int64, message string, files []repository.Hash, metadata map[string]string) (entity.CombinedId, *AddCommentOperation, error) {
	...
	i.Append(op)
	return entity.CombineIds(i.Id(), op.Id()), op, nil
}
