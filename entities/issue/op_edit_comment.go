...
// EditCommentOperation will change a comment in the issue
...
func EditComment(i Interface, author identity.Interface, unixTime int64, target entity.Id, message string, files []repository.Hash, metadata map[string]string) (entity.CombinedId, *EditCommentOperation, error) {
	...
	i.Append(op)
	return entity.CombineIds(i.Id(), target), op, nil
}

// EditCreateComment is a convenience function to edit the body of an issue (the first comment)
func EditCreateComment(i Interface, author identity.Interface, unixTime int64, message string, files []repository.Hash, metadata map[string]string) (entity.CombinedId, *EditCommentOperation, error) {
	createOp := i.FirstOp().(*CreateOperation)
	return EditComment(i, author, unixTime, createOp.Id(), message, files, metadata)
}
