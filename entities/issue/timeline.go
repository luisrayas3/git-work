...
// SetFieldTimelineItem records one field change, with the value it replaced.
type SetFieldTimelineItem struct {
	combinedId entity.CombinedId
	Author     identity.Interface
	UnixTime   timestamp.Timestamp
	Key        string
	Value      Value
	// Was is the previous value, nil when the field was unset.
	Was Value
}

func (s *SetFieldTimelineItem) CombinedId() entity.CombinedId {
	return s.combinedId
}

// ValueTimelineItem records one item added to, or removed from, a list-valued field.
type ValueTimelineItem struct {
	combinedId entity.CombinedId
	Author     identity.Interface
	UnixTime   timestamp.Timestamp
	Key        string
...
func (v *ValueTimelineItem) CombinedId() entity.CombinedId {
	return v.combinedId
}
